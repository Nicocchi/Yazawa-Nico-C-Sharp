...
namespace Yazawa_Nico.Modules.Images
{
    ...
    {
        [Command("sad", RunMode = RunMode.Async)]
        [Summary("sad | Be sad. Requires bot SendMessages and EmbedLinks permissions.")]
        [RequireBotPermission(GuildPermission.SendMessages)]
        [RequireBotPermission(GuildPermission.EmbedLinks)]
        public async Task Default([Remainder]string users = null)
        {
            string fileLoc = "JSON/sad.json"; // Location of the JSON file
            string message = "";
            string allUsers = "";
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(users))
                {
                    ...
                    message = $"**{Context.User.Username}** is sad from {allUsers}!";
                }
                else
                {
                    message = $" **{Context.User.Username}** is sad.";
                }

                EmbedBuilder builder = new EmbedBuilder()
                    .WithColor(new Color(255,73,167))
                    .AddField("ಠ__ಠ", message)
