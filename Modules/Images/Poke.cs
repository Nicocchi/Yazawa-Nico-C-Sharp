...
namespace Yazawa_Nico.Modules.Images
{
    public class Poke : ModuleBase<SocketCommandContext>
    {
        ...
        [Summary("poke | Poke someone. Requires bot SendMessages and EmbedLinks permissions.")]
        [RequireBotPermission(GuildPermission.SendMessages)]
        [RequireBotPermission(GuildPermission.EmbedLinks)]
        public async Task Default([Remainder]string users = null)
        {
            string fileLoc = "JSON/poke.json"; // Location of the JSON file
            string message = "";
            string allUsers = "";
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(users))
                {
                    ...
                    message = $"**{Context.User.Username}** is poking {allUsers}!";
                }
                else
                {
                    message = $" **{Context.User.Username}** is poking themself.";
                }

                EmbedBuilder builder = new EmbedBuilder()
                    .WithColor(new Color(255,73,167))
                    .AddField("POKE", message)
