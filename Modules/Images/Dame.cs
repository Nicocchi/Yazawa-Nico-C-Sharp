...
{
    public class No : ModuleBase<SocketCommandContext>
    {
        [Command("no", RunMode = RunMode.Async)]
        [Summary("no | Disagree. Requires bot SendMessages and EmbedLinks permissions.")]
        [RequireBotPermission(GuildPermission.SendMessages)]
        [RequireBotPermission(GuildPermission.EmbedLinks)]
        public async Task Default([Remainder]string users = null)
        {
            string fileLoc = "JSON/no.json"; // Location of the JSON file
            string message = "";
            string allUsers = "";
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(users))
                {
                    ...
                    message = $" **{Context.User.Username}**is saying no to {allUsers}.";
                }
                else
                {
                    message = $" **{Context.User.Username}** is saying no.";
                }
