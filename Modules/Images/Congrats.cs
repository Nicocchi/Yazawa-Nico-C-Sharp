...
{
    public class Congrats : ModuleBase<SocketCommandContext>
    {
        [Command("congrats", RunMode = RunMode.Async)]
        [Summary("congrats | Congratulate someone! Requires bot SendMessages and EmbedLinks permissions.")]
        [RequireBotPermission(GuildPermission.SendMessages)]
        [RequireBotPermission(GuildPermission.EmbedLinks)]
        public async Task Default([Remainder]string users = null)
        {

            string fileLoc = "JSON/congrats.json"; // Location of the JSON file
            string message = "";
            string allUsers = "";
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(users))
                {
                    ...
                    message = $" **{Context.User.Username}**is congratulating {allUsers}.";
                }
                else
                {
                    message = $" **{Context.User.Username}** is congratulating themself.";
                }

                EmbedBuilder builder = new EmbedBuilder()
                    .WithColor(new Color(255,73,167))
                    .AddField("ヽ(＾Д＾)ﾉ", message)
