...
{
    public class Cry : ModuleBase<SocketCommandContext>
    {
        [Command("cry", RunMode = RunMode.Async)]
        [Summary("cry | Cry to yourself or from someone. Requires bot SendMessages and EmbedLinks permissions.")]
        [RequireBotPermission(GuildPermission.SendMessages)]
        [RequireBotPermission(GuildPermission.EmbedLinks)]
        public async Task Default([Remainder]string users = null)
        {
            string fileLoc = "JSON/cry.json"; // Location of the JSON file
            string message = "";
            string allUsers = "";
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(users))
                {
                    ...
                    message = $" **{Context.User.Username}**is crying from {allUsers}.";
                }
                else
                {
                    message = $" **{Context.User.Username}** is crying to themself.";
                }

                EmbedBuilder builder = new EmbedBuilder()
                    .WithColor(new Color(255,73,167))
                    .AddField("(༎ຶ⌑༎ຶ)", message)
