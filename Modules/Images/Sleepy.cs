...
{
    public class Sleepy : ModuleBase<SocketCommandContext>
    {
        [Command("sleepy", RunMode = RunMode.Async)]
        [Summary("sleepy | Be sleepy. Requires bot SendMessages and EmbedLinks permissions.")]
        [RequireBotPermission(GuildPermission.SendMessages)]
        [RequireBotPermission(GuildPermission.EmbedLinks)]
        public async Task Default([Remainder]string users = null)
        {
            string fileLoc = "JSON/sleepy.json"; // Location of the JSON file
            string message = "";
            string allUsers = "";
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(users))
                {
                    ...
                    message = $"**{Context.User.Username}** is sleepy with {allUsers}!";
                }
                else
                {
                    message = $" **{Context.User.Username}** is sleepy!";
                }

                EmbedBuilder builder = new EmbedBuilder()
                    .WithColor(new Color(255,73,167))
                    .AddField("(￣o￣) zzZZzzZZ", message)
