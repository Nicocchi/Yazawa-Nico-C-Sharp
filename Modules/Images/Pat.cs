...
namespace Yazawa_Nico.Modules.Images
{
    public class Pat : ModuleBase<SocketCommandContext>
    {
        ...
        [Summary("pat | Pat someone. Requires bot SendMessages and EmbedLinks permissions.")]
        [RequireBotPermission(GuildPermission.SendMessages)]
        [RequireBotPermission(GuildPermission.EmbedLinks)]
        public async Task Default([Remainder]string users = null)
        {
            string fileLoc = "JSON/pat.json"; // Location of the JSON file
            string message = "";
            string allUsers = "";
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(users))
                {
                    ...
                    message = $"**{Context.User.Username}** is patting {allUsers}!";
                }
                else
                {
                    message = $" **{Context.User.Username}** is patting themself...";
                }

                EmbedBuilder builder = new EmbedBuilder()
                    .WithColor(new Color(255,73,167))
                    .AddField("(ｏ・_・)ノ”(ノ_<、)", message)
