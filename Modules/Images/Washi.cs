...
{
    public class Washi : ModuleBase<SocketCommandContext>
    {
        [Command("washi", RunMode = RunMode.Async)]
        [Summary("washi | Fondle breasts. Requires bot SendMessages and EmbedLinks permissions.")]
        [RequireBotPermission(GuildPermission.SendMessages)]
        [RequireBotPermission(GuildPermission.EmbedLinks)]
        public async Task Default([Remainder]string users = null)
        {
            string fileLoc = "JSON/washi.json"; // Location of the JSON file
            string message = "";
            string allUsers = "";
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(users))
                {
                    ...
                    if (usernames.Contains("themself"))
                    {
                        message = $" **{Context.User.Username}** is fondling their own breasts...";
                    }
                    else
                    {
                        message = $"O///O **{Context.User.Username}** is fondling {allUsers} breasts!";
                    }
                }
                else
                {
