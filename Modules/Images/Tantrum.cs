...
{
    public class Tantrum : ModuleBase<SocketCommandContext>
    {
        [Command("tantrum", RunMode = RunMode.Async)]
        [Summary("tantrum | Throw a tantrum. Requires bot SendMessages and EmbedLinks permissions.")]
        [RequireBotPermission(GuildPermission.SendMessages)]
        [RequireBotPermission(GuildPermission.EmbedLinks)]
        public async Task Default([Remainder]string users = null)
        {
            string fileLoc = "JSON/tantrum.json"; // Location of the JSON file
            string message = "";
            string allUsers = "";
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(users))
                {
                    ...
                    message = $"**{Context.User.Username}** is throwing a tantrum from {allUsers}!";
                }
                else
                {
                    message = $" **{Context.User.Username}** is throwing a tantrum!";
                }
