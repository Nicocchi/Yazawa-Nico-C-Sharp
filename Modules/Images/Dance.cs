...
{
    public class Dance : ModuleBase<SocketCommandContext>
    {
        [Command("dance", RunMode = RunMode.Async)]
        [Summary("dance | Dance. Requires bot SendMessages and EmbedLinks permissions.")]
        [RequireBotPermission(GuildPermission.SendMessages)]
        [RequireBotPermission(GuildPermission.EmbedLinks)]
        public async Task Default([Remainder]string users = null)
        {
            string fileLoc = "JSON/dance.json"; // Location of the JSON file
            string message = "";
            string allUsers = "";
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(users))
                {
                    ...
                    message = $" **{Context.User.Username}**is dancing with {allUsers}.";
                }
                else
                {
                    message = $" **{Context.User.Username}** is dancing.";
                }

                EmbedBuilder builder = new EmbedBuilder()
                    .WithColor(new Color(255,73,167))
                    .AddField("٩(θ‿θ)۶", message)
