...
namespace Yazawa_Nico.Modules.Images
{
    public class Slap : ModuleBase<SocketCommandContext>
    {
        ...
        [Summary("slap | Slap someone. Requires bot SendMessages and EmbedLinks permissions.")]
        [RequireBotPermission(GuildPermission.SendMessages)]
        [RequireBotPermission(GuildPermission.EmbedLinks)]
        public async Task Default([Remainder]string users = null)
        {
            string fileLoc = "JSON/slap.json"; // Location of the JSON file
            string message = "";
            string allUsers = "";
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(users))
                {
                    ...
                    message = $"**{Context.User.Username}** is slapping {allUsers}!";
                }
                else
                {
                    message = $" **{Context.User.Username}** is slapping themself!";
                }
