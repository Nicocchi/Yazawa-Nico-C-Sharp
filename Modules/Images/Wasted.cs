...
{
    public class Wasted : ModuleBase<SocketCommandContext>
    {
        [Command("wasted", RunMode = RunMode.Async)]
        [Summary("wasted | Be wasted. Requires bot SendMessages and EmbedLinks permissions.")]
        [RequireBotPermission(GuildPermission.SendMessages)]
        [RequireBotPermission(GuildPermission.EmbedLinks)]
        public async Task Default([Remainder]string users = null)
        {
            string fileLoc = "JSON/wasted.json"; // Location of the JSON file
            string message = "";
            string allUsers = "";
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(users))
                {
                    ...
                    message = $"**{Context.User.Username}** wasted {allUsers}!";
                }
                else
                {
                    message = $" **{Context.User.Username}** is wasted!";
                }

                EmbedBuilder builder = new EmbedBuilder()
                    .WithColor(new Color(255,73,167))
                    .AddField("T_T", message)
