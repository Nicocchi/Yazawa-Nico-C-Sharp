...
{
    public class Highfive : ModuleBase<SocketCommandContext>
    {
        [Command("highfive", RunMode = RunMode.Async)]
        [Summary("highfive | Highfive someone!. Requires bot SendMessages and EmbedLinks permissions.")]
        [RequireBotPermission(GuildPermission.SendMessages)]
        [RequireBotPermission(GuildPermission.EmbedLinks)]
        public async Task Default([Remainder]string users = null)
        {
            string fileLoc = "JSON/highfive.json"; // Location of the JSON file
            string message = "";
            string allUsers = "";
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(users))
                {
                    ...
                    message = $" **{Context.User.Username}** is highfiving {allUsers}!";
                }
                else
                {
                    message = $" **{Context.User.Username}** is highfiving themself...";
                }

                EmbedBuilder builder = new EmbedBuilder()
                    .WithColor(new Color(255,73,167))
                    .AddField("ヽ(＾Д＾)ﾉヽ(＾Д＾)ﾉ", message)
