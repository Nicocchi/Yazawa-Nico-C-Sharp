...
{
    public class Badass : ModuleBase<SocketCommandContext>
    {
        [Command("badass", RunMode = RunMode.Async)]
        [Summary("badass | Be a badass or say a user(s) is being a badass. Requires bot SendMessages and EmbedLinks permissions.")]
        [RequireBotPermission(GuildPermission.SendMessages)]
        [RequireBotPermission(GuildPermission.EmbedLinks)]
        public async Task Default([Remainder]string users = null)
        {
            string fileLoc = "JSON/badass.json"; // Location of the JSON file
            string message = "";
            string allUsers = "";
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(users))
                {
                    ...
                    message = $" **{Context.User.Username}**is saying {allUsers} are/is being a badass!";
                }
                else
                {
                    message = $" **{Context.User.Username}** is being a badass!";
                }

                EmbedBuilder builder = new EmbedBuilder()
                    .WithColor(new Color(255,73,167))
                    .AddField("ლಠ益ಠ)ლ", message)
