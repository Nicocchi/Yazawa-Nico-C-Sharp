...
{
    [Summary("Be angry")]
    public class Angry : ModuleBase<SocketCommandContext>
    {
        [Command("angry", RunMode = RunMode.Async)]
        [Summary("angry | Be angry at oneself or a user. Requires bot SendMessages and EmbedLinks permissions.")]
        [RequireBotPermission(GuildPermission.SendMessages)]
        [RequireBotPermission(GuildPermission.EmbedLinks)]
        public async Task AngryUser([Remainder]string users = null)
        {
            string fileLoc = "JSON/angry.json"; // Location of the JSON file
            string message = "";
            string allUsers = "";
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(users))
                {
                    ...
                    message = $" **{Context.User.Username}** is angry at {allUsers}!";
                }
                else
                {
                    message = $" **{Context.User.Username}** is angry!";
                }
