...
namespace Yazawa_Nico.Modules.Images
{
    public class Hug : ModuleBase<SocketCommandContext>
    {
        ...
        [Summary("hug | Hug someone!. Requires bot SendMessages and EmbedLinks permissions.")]
        [RequireBotPermission(GuildPermission.SendMessages)]
        [RequireBotPermission(GuildPermission.EmbedLinks)]
        public async Task Default([Remainder]string users = null)
        {
            string fileLoc = "JSON/hug.json"; // Location of the JSON file
            string message = "";
            string allUsers = "";
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(users))
                {
                    ...
                    message = $"Awwwe >///< **{Context.User.Username}** is hugging {allUsers}!";
                }
                else
                {
                    message = $" **{Context.User.Username}** is hugging themself...";
                }

                EmbedBuilder builder = new EmbedBuilder()
                    .WithColor(new Color(255,73,167))
                    .AddField("(つ≧▽≦)つ", message)
