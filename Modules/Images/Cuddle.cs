...
namespace Yazawa_Nico.Modules.Images
{
    public class Cuddle : ModuleBase<SocketCommandContext>
    {
        ...
        [Summary("cuddle | Cuddle yourself or someone. Requires bot SendMessages and EmbedLinks permissions.")]
        [RequireBotPermission(GuildPermission.SendMessages)]
        [RequireBotPermission(GuildPermission.EmbedLinks)]
        public async Task Default([Remainder]string users = null)
        {
            string fileLoc = "JSON/cuddle.json"; // Location of the JSON file
            string message = "";
            string allUsers = "";
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(users))
                {
                    ...
                    message = $" **{Context.User.Username}**is cuddling {allUsers}.";
                }
                else
                {
                    message = $" **{Context.User.Username}** is cuddling themself.";
                }

                EmbedBuilder builder = new EmbedBuilder()
                    .WithColor(new Color(255,73,167))
                    .AddField("CUDDLE", message)
