...
namespace Yazawa_Nico.Modules.Images
{
    public class Lewd : ModuleBase<SocketCommandContext>
    {
        ...
        [Summary("lewd | Lewd someone!. Requires bot SendMessages and EmbedLinks permissions.")]
        [RequireBotPermission(GuildPermission.SendMessages)]
        [RequireBotPermission(GuildPermission.EmbedLinks)]
        public async Task Default([Remainder]string users = null)
        {
            string fileLoc = "JSON/lewd.json"; // Location of the JSON file
            string message = "";
            string allUsers = "";
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(users))
                {
                    ...
                    message = $"**{Context.User.Username}** is lewding {allUsers}!";
                }
                else
                {
                    message = $" **{Context.User.Username}** is being lewd!";
                }

                EmbedBuilder builder = new EmbedBuilder()
                    .WithColor(new Color(255,73,167))
                    .AddField("O///O", message)
