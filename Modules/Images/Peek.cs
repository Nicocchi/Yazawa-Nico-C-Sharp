...
{
    public class Peek : ModuleBase<SocketCommandContext>
    {
        [Command("peek", RunMode = RunMode.Async)]
        [Summary("peek | Peek. Requires bot SendMessages and EmbedLinks permissions.")]
        [RequireBotPermission(GuildPermission.SendMessages)]
        [RequireBotPermission(GuildPermission.EmbedLinks)]
        public async Task Default([Remainder]string users = null)
        {
            string fileLoc = "JSON/peek.json"; // Location of the JSON file
            string message = "";
            string allUsers = "";
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(users))
                {
                    ...
                    message = $"**{Context.User.Username}** is peeking at {allUsers}!";
                }
                else
                {
                    message = $" **{Context.User.Username}** is peeking.";
                }

                EmbedBuilder builder = new EmbedBuilder()
                    .WithColor(new Color(255,73,167))
                    .AddField("(o-_-o)", message)
