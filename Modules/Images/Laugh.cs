...
{
    public class Laugh : ModuleBase<SocketCommandContext>
    {
        [Command("laugh", RunMode = RunMode.Async)]
        [Summary("laugh | Laugh!. Requires bot SendMessages and EmbedLinks permissions.")]
        [RequireBotPermission(GuildPermission.SendMessages)]
        [RequireBotPermission(GuildPermission.EmbedLinks)]
        public async Task Default([Remainder]string users = null)
        {
            string fileLoc = "JSON/laugh.json"; // Location of the JSON file
            string message = "";
            string allUsers = "";
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(users))
                {
                    ...
                    message = $"**{Context.User.Username}** is laughing with {allUsers}!";
                }
                else
                {
                    message = $" **{Context.User.Username}** is laughing!";
                }

                EmbedBuilder builder = new EmbedBuilder()
                    .WithColor(new Color(255,73,167))
                    .AddField("｡ﾟ( ﾟ^∀^ﾟ)ﾟ｡", message)
