...
{
    [Summary("Bite someone")]
    public class Bite : ModuleBase<SocketCommandContext>
    {
        [Command("bite", RunMode = RunMode.Async)]
        [Summary("bite | Bite a user or users. Requires bot SendMessages and EmbedLinks permissions.")]
        [RequireBotPermission(GuildPermission.SendMessages)]
        [RequireBotPermission(GuildPermission.EmbedLinks)]
        public async Task Default([Remainder]string users = null)
        {
            string fileLoc = "JSON/bite.json"; // Location of the JSON file
            string message = "";
            string allUsers = "";
            ...
            try
            {
                ...
                {
                    message = $" **{Context.User.Username}** is biting themself! Ouch why would you do that!? >~<";
                }
